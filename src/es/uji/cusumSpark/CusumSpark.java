    ...
    private void cusumGUI(double l0, double b0, double b1, int lon, int lon2, int nven){
        ...
        while(true) {
            ...
            if (fase == 2) {
                ...
                for (int i = puntoCambio; i <= inicio; i++) {
                    aux.add((double)i);
                    dataAux.add(data.get(i-1));
                }
            ...
            while (fase == 2) {
                ...
                if (FuncionesAuxiliares.poissonFunction(dato, lbefore) != 0) { //FIXME esto no está en R
                    ...
                    if (!alarma && (ga.get(ga.size() - 1) > threshold || gb.get(gb.size() - 1) > threshold)) {
                        alarmi = dataIndex;
            ...
            if (fase == 3) {
                fin = dataIndex;
                double lv = lambda0 + beta0 * (medio-puntoCambio);
                ...
                // Doy una nueva pasada con la velocidad calculada
                lv = lambda0 + beta0 * (medio-puntoCambio);
